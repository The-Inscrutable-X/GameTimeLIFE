...
class GXP():
    def __init__(self, name, xp, parent=None, isTask=False):
        ...
        self.parent = parent
        self.isTask = isTask
        self.level = (lambda x: x.level + 1 if x != None else 0)(
            self.parent
        )  #finds which hierarchical level the node/leaf is on upon creation by counting parents/earlier nodes
    ...
    # obtain references to all nodes in tree semirecursively, what is this called?
    def get_cum_boys(
            self,
            origin):  # the origin argument is obsolete and has no function
        cum_boys = [self]
        for i in self.boys:
    ...
    with open('tree_storage.json', 'r') as json_tree:
        ...
        for node in tree:
            ...
            try:
                ...
                TorN = False
            Root.find(parent).insert((name, xp, TorN))
    return Root
...
class Task(GXP):
    def __init__(self, name, xp, parent=None, isTask=True):
        super().__init__(name=name, xp=xp, parent=parent, isTask=isTask)
        self.description = ""
        self.time_limit = None
        self.repeating = None
        self.rep_frequency = None


#for handeling tree style printing
...
def print_xp_tree(Root, gui_mode=False):
    ...
    for i in Root.get_cum_boys(Root):
        level_dict[i.level] = []
    for i in Root.get_cum_boys(Root):
        level_dict[i.level].append(
            i
        )  #appends nodes into print format, must exclude tasks from this level dict
    # print(level_dict)
    for level, nodes in level_dict.items():
        ...
        for j in range(0, len(nodes) + 1):
            ...
            for i, l in enumerate(i_locations):
                if j == -1:
                    string_list = replacer(string_list,
                                           str(nodes[i].cached_xp), l)
                    pass
